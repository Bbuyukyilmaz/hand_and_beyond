...
typedef struct {
    S_EMGChannel channels[NUM_EMG_CHANNELS];
} S_EMGRecorder;

typedef struct {
    double MAV;		// Mean Absolute Value (MAV)
    double WL;		// Waveform Length (WL)
    int ZC;			// Zero Crossings (ZC)
    int SSC;		// Slope Sign Changes (SSC)
    double RMS;		// Root Mean Square (RMS)
} EMGFeatures;
